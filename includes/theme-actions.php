...
function woo_custom_styling() {
	...
	if ( isset( $woo_options['woo_style_disable'] ) && $woo_options['woo_style_disable'] != 'true' ) {} else {
		...
		if ( $nav_font )
			$output .= 'ul.nav li a, #navigation ul.rss a, #navigation ul.cart a.cart-contents, #navigation .cart-contents #navigation ul.rss, #navigation ul.nav-search, #navigation ul.nav-search a { ' . woo_generate_font_css( $nav_font, 1 ) . ' } #navigation ul.rss li a:before, #navigation ul.nav-search a.search-contents:before { color:' . $nav_font['color'] . ';}' . "\n";
		if ( $nav_hover )
...
function woo_post_meta() {
	...
	$post_info = '[post_author_posts_link] <span class="small">' . _x( 'on', 'post datetime', 'woothemes' ) . '</span> [post_date] <span class="small">' . __( 'in', 'woothemes' ) . '</span> [post_categories before=""] ';
